...
class SocketQueue extends socket {
    ...
    initWSocket() {
        try {
            ...
            this.socket.onmessage = (evt) => {
                ...
                log_1.default.Info(`WebSocket status '${evt.type}', New Message - ${utils_1.default.getString(data)}`);
                this.message(evt);
            };
            this.socket.onerror = (err) => {
                log_1.default.Error(`WebSocket status '${err.type}' - ${utils_1.default.getString(err.reason, '未知错误')}`);
                this.WSState = err.target.readyState;
                this.rebuildSocket(err.type);
                this.error(err);
            };
            this.socket.onclose = (evt) => {
                log_1.default.Warn(`WebSocket status '${evt.type}' - ${utils_1.default.getString(evt.reason, '未知错误导致关闭')}`);
    ...
    init(options) {
        ...
        if (utils_1.default.isObject(socket)) {
            ...
            utils_1.default.setFunction(['open', 'closed', 'error', 'message', 'reConnect'], this, socket);
        }
